...
class NotificationController extends Controller
{
    ...
    public function index()
    {
        ...
        return view(
            ...
            compact("notifications", "events", "roles"),
        );
    }
    ...
    public function store(Request $request)
    {
        $validated = $request->validateWithBag("notification", [
            ...
            "message" => "string",
            "target_type" => "required|string|in:event,role,participant",
            ...
            "target_participants" =>
                "array|required_if:target_type,participant",
            "target_participants.*" => "nullable|exists:event_participants,id",
            "status" => "required|in:draft,scheduled,sent",
            "schedule_date" => "nullable|required_if:status,scheduled|date",
            "schedule_time" =>
                "nullable|required_if:status,scheduled|date_format:H:i",
        ]);
    ...
    public function import(Request $request)
    {
        ...
        foreach ($rows as $index => $row) {
            ...
            $data["target_events"] = !empty($data["target_events"])
                ? array_filter(explode(",", $data["target_events"]))
                : [];
            $data["target_roles"] = !empty($data["target_roles"])
                ? array_filter(explode(",", $data["target_roles"]))
                : [];
            $data["target_users"] = !empty($data["target_users"])
                ? array_filter(explode(",", $data["target_users"]))
                : [];
        ...
        if ($errors->isNotEmpty()) {
            return back()->withErrors($errors, "import")->withInput();
        }
            ...
            DB::transaction(function () use ($validated) {
                ...
                switch ($validated["target_type"]) {
                    case "event":
                        $notification
                            ->events()
                            ->sync($validated["target_events"]);
                        break;
                    case "role":
                        $notification
                            ->roles()
                            ->sync($validated["target_roles"]);
                        break;
                    case "user":
                        $notification
                            ->users()
                            ->sync($validated["target_users"]);
                        break;
                }
            });
        ...
        return redirect()
            ...
            ->with(
                "success",
                "$importedCount notification(s) imported successfully.",
            )
